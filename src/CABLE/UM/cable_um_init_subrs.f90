   ...
   subroutine initialize_maps(latitude,longitude, tile_index_mp)
      ...
      real, dimension(um1%row_length,um1%rows) :: &
         ...
         !if ( cable_user%initialize_mapping ) then
            ...
            !fudge and send asinlatitude to latitude
          ...
          !if(knode_gl==1) then  
            open(unit=1251,file='clattitude',status="unknown", &
                  action="write", form="formatted",position='append' )
                  !WRITE(1251,*) , cable%const%math%pi180
                  WRITE(1251,*) , "" 
                  do i=1, um1%row_length      
                     do j=1, um1%rows     
                        WRITE(1251,*) , i,j, acoslat(i,j)
                     enddo   
                  enddo   
            close(1251)

            open(unit=12511,file='c_init_lat',status="unknown", &
                  action="write", form="formatted",position='append' )
                  !WRITE(1251,*) , cable%const%math%pi180
                  WRITE(12511,*) , "" 
                  do i=1, um1%row_length      

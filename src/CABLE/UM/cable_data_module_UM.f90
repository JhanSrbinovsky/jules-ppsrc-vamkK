...
End interface cond_print
   ...
   !real, dimension(land_pts,ntiles,ms), pointer, save ::                       &
   real, dimension(:,:,:), pointer, save ::                       &
      TSOIL_TILE_diag, &
      SMCL_TILE_diag,  &
      STHF_TILE_diag

   !real, dimension(land_pts,ntiles,msn), pointer, save ::                      & 
   real, dimension(:,:,:), pointer, save ::                      & 
      SNOW_DEPTH3L_diag,  &
      SNOW_MASS3L_diag,  &
      SNOW_TMP3L_diag,  &
   ...
   !real, dimension(land_pts,ntiles), pointer, save ::                          &
   real, dimension(:,:), pointer, save ::                          &
      SNOW_RHO1L_diag, &
      SNOW_AGE_diag,   &
      SNOW_FLG3L_diag
...
SUBROUTINE cable_atm_step( mype, UM_eq_TRUE, L_cable, a_step, timestep_len, row_length,     &
   ...
   REAL, DIMENSION(row_length, rows), TARGET :: &
      latitude, longitude
...
!      flatitude, flongitude
   ...
   !---------------------------------------------------------------------------
print *, "land_index: ", shape(land_index)
...
SUBROUTINE cable_control5( alb_tile, land_albedo,         &
   ...
   !Real, dimension(cable% mp% rows, cable% mp% row_length,4, cable% mp% n_swcall), target ::                 &
...
!SUBROUTINE cable_glue_rad_init( surf_down_sw, n_swcall )
